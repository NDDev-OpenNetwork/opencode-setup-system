...
declares.** The rule a model reads while working on OpenCode said the product
...
**A walk written when a surface held one kind.** OpenCode's table gained a
...
**OpenCode's table gained a kind.** Where a product loads a plugin from a
...
**OpenCode declares the `instruction` kind**, and the surface that routes it is
...
OpenCode's provider publishes a projection profile per *scope* -- a second
...
Starts OpenCode, and takes over a target the estate before this one
still claims.
...
OpenCode harness configuration in a caller-named target directory.
