...
opencode-setup-system select minimal      --target ~/.tool-config
...
opencode-setup-system restore --backup slot-000000000002 --target ~/.tool-config
...
docker run --rm -v "$HOME/.config:/config" \
  ghcr.io/nddev-opennetwork/opencode-setup-system:0.0.3 \
